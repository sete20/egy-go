                                ...
                                <div class="my-2 col-md-4 my-md-0">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <select name="client_id" class="form-control" id="kt_datatable_search_status">
                                            ...
                                            <option @if(isset($_GET['client_id']) && $_GET['client_id']==$client->id) selected @endif value="{{$client->id}}">{{$client->name ? $client->name : ''}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
            ...
            <thead>
                <tr>
                    ...
                    @if($status != "all") <th class="text-center">{{translate('Options')}}</th> @endif
                       @if($status ==\App\Shipment::DELIVERED_STATUS) <th class="text-center">{{translate(' التوريد في الفرع')}}</th> @endif
                </tr>
            </thead>
            <tbody>
                @php
                    ...
                    @if($client_id != $shipment->client_id)
                        <tr class="bg-light">
                            ...
                            <th colspan="4">
                                ...
                                @else
                                    {{$shipment->client->name ? $shipment->client->name : ''}}
                                @endif
                            </th>
                        </tr>
                        @php
                    ...
                    <tr>
                        <td>
                            ...
                            @else
                                <label class="checkbox checkbox-success">
                                    <input data-missionid="{{$shipment->mission_id}}"
                                     data-clientaddresssender="{{$shipment->client_address}}"
                                      data-clientaddress="{{$shipment->reciver_address}}"
                                      data-clientname="{{$shipment->reciver_name}}"
                                       data-clientstatehidden="{{$shipment->to_state_id}}"
                                        data-clientstate="{{$shipment->to_state->name ?? '' }}"
                                         data-clientareahidden="{{$shipment->to_area_id}}"
                                         data-clientarea="{{$shipment->to_area->name ?? '' }}"
                                          data-clientid="{{$shipment->client->id}}"
                                           data-paymentmethodid="{{$shipment->payment_method_id}}"
                                            data-branchid="{{$shipment->branch_id ?? ''}}"
                                            data-branchname="{{$shipment->branch->name ?? ''}}"
                                             type="checkbox" class="sh-check checkbox-client-id-{{$shipment->client_id}}"
                                    name="checked_ids[]" value="{{$shipment->id}}" /><span></span></label>
                            @endif
                        </td>
                        @if($user_type == 'admin' || $auth_user->userClient->client_id == $shipment->client_id || in_array('1100', $staff_permission) || in_array('1009', $staff_permission) )
                        ...
                        @if( in_array($user_type ,['admin','customer']) || in_array('1100', $staff_permission) || in_array('1006', $staff_permission) )
                            <td>
                                ...
                                @else
                                <a href="#">please selec branch for the current shipment</a>
                                @endif
                            </td>
                        @else
                        ...
                        <td>{{$shipment->shipping_date}}</td>
                            @if($status == \App\Shipment::CAPTAIN_ASSIGNED_STATUS || $status == \App\Shipment::RECIVED_STATUS)
                            @if($shipment->captain_id)

                            <td><a href="{{route('admin.captains.show', $shipment->captain_id)}}">@isset($shipment->captain_id) {{$shipment->captain->name ?? $shipment->captain->name}} @endisset</a></td>
                            @endif
                            @endif
                        @if($status == \App\Shipment::APPROVED_STATUS || $status == \App\Shipment::CAPTAIN_ASSIGNED_STATUS || $status == \App\Shipment::RECIVED_STATUS )
                        ...
                        @if($status != "all")
                            <td class="text-center">
                                ...
                                @endif
                                            @if($status != \App\Shipment::APPROVED_STATUS && $status != \App\Shipment::CAPTAIN_ASSIGNED_STATUS && $status != \App\Shipment::CLOSED_STATUS && $status != \App\Shipment::RECIVED_STATUS && $status != \App\Shipment::IN_STOCK_STATUS && $status != \App\Shipment::DELIVERED_STATUS && $status != \App\Shipment::SUPPLIED_STATUS && $status != \App\Shipment::RETURNED_STATUS )
                                    <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('admin.shipments.delete', $shipment->id)}}" title="{{ translate('حذف') }}">
                        ...
                        @endif
                                     @if ($status == \App\Shipment::DELIVERED_STATUS )
                                     <td>
                                           @if($shipment->mission_id == null)
                                              <a  class="btn btn-primary" href="{{ route('admin.shipment.supply',$shipment) }}" >
                                                 {{translate('توريد ')}}
                                             </a>
                                             @else
                                             <button  class="btn btn-primary" disabled>
                                                 {{translate('جاري التوريد')}}
                                             </button>
                                             @endif
                                        </td>
                                @endif
