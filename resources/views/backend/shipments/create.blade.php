                    ...
                    <div class="col-lg-12">
                        ...
                        <hr>
                            ...
                            <div class="col-9 col-form-label">
                                <div class="radio-inline">
                                    <label class="radio radio-success btn btn-default">
                                        <input  type="radio" name="Shipment[package_return]"  value="1" />
                                        <span></span>
                                        {{translate("نعم")}}
                                    </label>
                                    <label class="radio radio-success btn btn-default">
                                        <input checked="checked" type="radio" name="Shipment[package_return]" value="0" />
                                        <span></span>
                                        {{translate("لا")}}
                                    </label>
                                </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    @php
                                              $client = \App\Client::where('id', $user_client)->first();
                                    $state = \App\State::where('id', $client->state_id)->with('branch')->first();

                                    @endphp
                                       @endif
                                         @if(Auth::user()->user_type== 'customer')
                                         <select class="form-control kt-select2 select-branch" disabled name="Shipment[branch_id]">
                                         @else
                                        ...
                                        $client = \App\Client::where('id', $user_client)->first();
                                        $state = \App\State::where('id', $client->state_id)->with('branch')->first();
                                        <option selected disabled value="{{ $state->branch->id }}">{{ $state->branch->name }}</option>
                                        @else
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    @if(Auth::user()->user_type== 'customer')
                                    @php
                                    $client = \App\Client::find(Auth::user()->userClient->client_id)->first();
                                    @endphp
                                    <input placeholder="{{translate('Client Phone')}}" name="Shipment[client_phone]" value="{{ $client-> responsible_mobile}}"  disabled id="client_phone" class="form-control" id="" />
                                    @else
                                    <input placeholder="{{translate('Client Phone')}}" name="Shipment[client_phone]"  disabled id="client_phone" class="form-control" id="" />
                                    @endif
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    $client = \App\Client::find(Auth::user()->userClient->client_id)->first();
                                    @endphp
                                    <input id="client-addressess" name="Shipment[client_address]" value="{{ $client-> address}}"  disabled class="form-control"  />
                                    @else
                                    <input id="client-addressess"  name="Shipment[client_address]"  disabled class="form-control"  />
                                    @endif
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control kt-select2 payment-type" disabled id="payment_type" name="Shipment[payment_type]">
                                        <option  selected  value="1">{{translate('Postpaid')}}</option>
                                    </select>
                                    ...
                                    <div class="text-left modal-body">
                                        <div class="row">
                                            @if($is_def_mile_or_fees=='2')
                                                ...
                                                <div class="col-6 shipment_cost" id="shipping_cost"></div>
                                            @elseif($is_def_mile_or_fees=='1')
                                                <div class="col-6">{{translate('Mile Cost')}} :</div>
                                                <div class="col-6 shipment_cost" id="mile_cost" ></div>
                                            @endif
                                        </div>
                                        <div class="row">
...
<script type="text/javascript">
    ...
    function getAdressess(client_id)
    {
        ...
        $.get("{{route('admin.shipments.get-addressess-ajax')}}?client_id=" + id, function(data) {
            if(data.length != 0){
                console.log(data);
                $('input[name ="Shipment[client_address]"]').val(data.address);
    ...
    $('#shipment_price').change(function() {
      var shipment_price = $('#shipment_price').find(":selected").val();
      var amount_to_be_collected = $('.amount_to_be_collected').val();
      var supplyInput = $("input[name='Shipment[amount_to_be_supply]']");
      console.log(supplyInput);
        if(amount_to_be_collected !=0 ){

          var amount_to_be_supply =  amount_to_be_collected - shipment_price;
        supplyInput.val(0);
        supplyInput.val(amount_to_be_supply);
        }
    });
    ...
    $('.select-client').change(function(){
        ...
        var client_address = $(this).find(':selected').data('address');
    ...
    function get_estimation_cost() {
      ...
      var shipment_price = $('#shipment_price').find(":selected").val();
            ...
            if({{$is_def_mile_or_fees}} =='2')
            {
                document.getElementById("shipping_cost").innerHTML = shipment_price;
                document.getElementById("return_cost").innerHTML = response.return_cost;
            }else if({{$is_def_mile_or_fees}} =='1')
            {
                document.getElementById("mile_cost").innerHTML = shipment_price;
      ...
      var amount_to_be_collected = $('.amount_to_be_collected').val();
            ...
            document.getElementById("total_cost").innerHTML = amount_to_be_collected;
