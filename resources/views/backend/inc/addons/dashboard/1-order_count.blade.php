...
@elseif($user_type == 'branch')
    @php
        ...
        $transactions                   = App\Transaction::where('client_id', Auth::user()->userBranch->branch_id)->where('status','none')->orderBy('created_at','desc')->sum('value');
        $transactionsDue                   =
        App\Transaction::where('client_id', Auth::user()->userBranch->branch_id)->where('status','none')->
        where('type','Due')->  orderBy('created_at','desc')->sum('value');
    @endphp
   <div class="row">
        <div class="col-xl-12">
            <!--begin::Stats Widget 30-->
            <div class="card card-custom bgi-no-repeat card-stretch gutter-b">
             ...
             <hr>
                                 ...
                                 <a href="#" class="card-title font-weight-bold text-muted text-hover-primary font-size-h5">{{translate('المبالغ المستحقة عل الفرع')}}</a>
                    <div class="mb-5 font-weight-bold text-success mt-9">{{format_price($transactionsDue)}}</div>
             <hr>
             <div class="col-3">
                <button  class="btn btn-sm btn-primary supply-order" data-id="{{ Auth::user()->userBranch->branch_id }}" style="margin: 8px" @if($transactions < 1) aria-disabled="true" disabled  @endif> طلب تـوريد</a>
             </div>
            <div class="col-3">
...
@elseif($user_type == 'customer')
    @php
        ...
        $transactions                   = App\Transaction::where('client_id', Auth::user()->userClient->client_id)->where('status','none')->orderBy('created_at','desc')->sum('value');
    @endphp
    ...
    <div class="row">
        <div class="col-xl-12">
            ...
            <div class="card card-custom bgi-no-repeat card-stretch gutter-b">
             ...
             <hr>
             <div class="col-3">
                <button  class="btn btn-sm btn-primary supply-order" data-id="{{ Auth::user()->userClient->client_id }}" style="margin: 8px" @if($transactions < 1) aria-disabled="true" disabled  @endif> طلب تـوريد</a>
                <button  class="btn btn-sm btn-primary pickup-order" data-id="{{ Auth::user()->id }}" style="margin: 8px" > طلب بيـك اب</a>
...
@elseif($user_type == 'captain')
    @php
        $transactions = App\Transaction::where('captain_id', Auth::user()->userCaptain->captain_id)->where('status','none')->orderBy('created_at','desc')->sum('value');
    ...
    <div class="row">
      <div class="col-xl-12">
            ...
            <div class="card card-custom bgi-no-repeat card-stretch gutter-b">
             ...
             <hr>
             <div class="col-3">
                <button  class="btn btn-sm btn-primary supply-order" data-id="{{ Auth::user()->userCaptain->captain_id }}" style="margin: 8px" @if($transactions < 1) aria-disabled="true" disabled  @endif> طلب تـوريد</a>
...
<script>
  ...
  $('.supply-order').click(function(){
        $.ajax({
             type: 'POST',
            url:"{{ route('supply.order.client') }}",
            data:{
